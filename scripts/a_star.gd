...
func _add_points(pathables: Array):
	for pathable in pathables:
		var mesh : MeshInstance3D = pathable.get_node("MeshInstance3D")
		var aabb: AABB = mesh.mesh.get_aabb()
		var start_point = aabb.position + pathable.global_position
...
func _connect_points():
	for point in points:
		...
		for neighbor_id in adjacent_points:
			if current_id != neighbor_id and not astar.are_points_connected(current_id, neighbor_id):
